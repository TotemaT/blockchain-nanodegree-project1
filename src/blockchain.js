...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        console.error('Could not add block to the blockchain', e);
        reject(e);
      }
    });
  }
  ...
  requestMessageOwnershipVerification(address) {
    ...
    return new Promise((resolve) => {
      resolve(`${address}:${self._timestamp()}:starRegistry`);
    });
  }
  ...
  submitStar(address, message, signature, star) {
    let self = this;
    return new Promise(async (resolve, reject) => {
      const starTimestamp = parseInt(message.split(':')[1]);
      const currentTime = self._timestamp();
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      resolve(self.chain.find((elt) => elt.hash === hash));
    });
  }
  ...
  validateChain() {
    ...
    return new Promise(async (resolve, reject) => {
      for (let i = self.height; i > 0; i--) {
        ...
        if (
          block.previousBlockHash !== (await self.getBlockByHeight(i - 1)).hash
        ) {
          errorLog.push(
            `Block ${block.hash} contains an invalid previous block hash.`
          );
        }
      }
